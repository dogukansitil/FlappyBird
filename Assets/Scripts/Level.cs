...
public class Level : MonoBehaviour
{
    ...
    private const float PIPE_HEAD_HEIGT = 3.75f;
    ...
    {
        //CMDebug.TextPopupMouse("Öldün Çık!");
    ...
    private void SpawnInitialClouds()
    {
        ...
        float cloudY = +47.5f;
        Transform cloudTransform = Instantiate(GameAssets.GetInstance().pfCloud_1, new Vector3(0, cloudY, 0), Quaternion.identity);
        cloudList.Add(cloudTransform);
    }

    private void HandleClouds()
    {
        // Handle Cloud Spwaning
    ...
    private void HandleGround()
    {
        foreach (Transform groundTransform in groundList)
        {
            ...
            if (groundTransform.position.x < GROUND_DESTROY_X_POSITION)
            {
                ...
                //Place Ground on the right most
                float groundWidth = 192f;
    ...
    private void SetDiffuculty(Difficulty difficulty)
    {
        switch (difficulty)
        {
    ...
    private void CreatePipe(float height, float xPosition, bool createBottom)
    {
        ...
        if (createBottom)
        {
            pipeHeadYPosition = -CAMERA_ORTHO_SIZE + height - PIPE_HEAD_HEIGT * .5f;
        }
        else
        {
            pipeHeadYPosition = +CAMERA_ORTHO_SIZE - height + PIPE_HEAD_HEIGT * .5f;
